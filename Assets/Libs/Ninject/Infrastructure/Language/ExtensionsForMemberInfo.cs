...
namespace Ninject.Infrastructure.Language
{
    ...
    public static class ExtensionsForMemberInfo
    {
		...
		private static PropertyInfo GetParentDefinition (PropertyInfo property)
		{
			...
			PropertyInfo result;
			if (methodInfo != null) {
				methodInfo = methodInfo.GetParentDefinition (BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
				if (methodInfo != null) {
					result = methodInfo.GetPropertyFromDeclaredType (property, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
					return result;
				}
			}
			result = null;
		...
		private static MethodInfo GetParentDefinition (this MethodInfo method, BindingFlags flags)
		{
			...
			while (methodInfo == null && baseType != null) {
				methodInfo = (from m in baseType.GetMethods (flags)
					where m.GetBaseDefinition ().Equals (baseDefinition)

...
public class LanguageEditor : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("Update translations"))
        {
            ...
            if (gDocsURL.Contains("pubhtml"))
                gDocsURL = gDocsURL.Replace("pubhtml", "pub");
    ...
    void LoadSettings(string gDocsPage)
    {
        ...
        gDocsPage += "&timestamp=" + EditorApplication.timeSinceStartup;//Prevent caching
        ...
        for (int i = sheetIDs.Count - 1; i >= 0; i--)
        {
            ...
            string thisURL = gDocsPage.Replace("gid=0", "gid=" + (downloadSheet)) + "&" + EditorApplication.timeSinceStartup;
            //Debug.Log(thisURL);
            ...
            if (data != "")
            {
                ...
                if (!ParseData(data, sheetIDs[i].title))
                {
                    //Failed
                }
            }
            ...
            {
                //crucialErrors++;
                //Debug.LogError("Unable to receive data");
            }
        }
        if (foundSheets == 0)
        {
    ...
    bool ParseData(string data, string sheetTitle)
    {
        ...
        for (int i = 0; i < lines.Count; i++)
        {
            ...
            else if (contents.Count > 1)
            {
                ...
                if (key == "") continue; //Skip entries with empty keys (the other values can be used as labels)
        ...
        if (!sheetNames.Contains(sheetTitle))

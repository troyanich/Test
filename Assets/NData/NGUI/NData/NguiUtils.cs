...
public static class NguiUtils
{
	...
	public static T GetComponentAs<T>(this Transform transform)
		where T : class
	{
		var mbs = transform.GetComponents<Component>();
		foreach(var mb in mbs)
		{
			if (mb is T)
				return mb as T;
	...
	public static void SetVisible(Transform transform, bool visible,  bool controlEffects = false)
	{
		SetNguiVisible (transform, visible,controlEffects);
...
//			}
	...
	private static void SetNguiVisible(GameObject gameObject, bool visible)
	{
		...
		foreach (var g in gameObject.GetComponents<UnityEngine.UI.Graphic>()) {
			g.enabled = visible;
		}
	}

	private static void SetNguiVisible(Transform transfom, bool visible,  bool controlEffects = false)
	{
		foreach(var collider in transfom.GetComponents<Collider>())
		{
			collider.enabled = visible;	
		}

		foreach(var widget in transfom.GetComponents<UIWidget>())
		{
			widget.enabled = visible;
		}

		//TODO: вынести в отдельную функцию
		foreach (var g in transfom.GetComponents<Graphic>()) {
			g.enabled = visible;
		}

        foreach (var rend in transfom.GetComponents<Renderer>())
        {
            rend.enabled = visible;
        }

		if (controlEffects) {
			foreach (var ps in transfom.GetComponents<ParticleSystem>()) {
				if (visible) {
					ps.Play ();
				} else {
					ps.Stop ();
        ...
        foreach (var sel in transfom.GetComponents<Selectable>())
        {
            sel.enabled = visible;
        }

        foreach (var lem in transfom.GetComponents<LayoutElement>())
        {
            lem.ignoreLayout = !visible;
        }
	}

	public static void SetSelectableEnabled(Transform transform, bool enabled,  IEnumerable<Transform> ignoreChild = null)
	{
		...
		for (var i = 0; i < transform.childCount; ++i)
		{
			...
			if (ignoreChild == null || !ignoreChild.Any () || !ignoreChild.All (ic => ic = child)) {
				foreach (var c in child.GetComponentsInChildren<Selectable>()) {
					c.interactable = enabled;
					SetSelectableEnabled (c.transform, enabled);
				}
			}
		}
	...
	public static void SetNguiColliderEnabledInChildren(Transform transform, bool enabled, IEnumerable<Transform> ignoreChild = null)
	{
		var collider = transform.GetComponent<Collider> ();
		if (collider != null) {
		...
		{
			var child = transform.GetChild(i);
            if(ignoreChild==null || !ignoreChild.Any(ic=>ic==child))
            {
                foreach (var c in child.GetComponentsInChildren<Collider>())
                {
                    c.enabled = enabled;
                    SetNguiColliderEnabledInChildren(c.transform, enabled);
                }
            }
    ...
    public static void SetUguiButtonsEnabledInChildren(Transform transform, bool enabled, IEnumerable<Transform> ignoreChild = null)
    {
        ...
        for (var i = 0; i < transform.childCount; ++i)
        {
            ...
            if (ignoreChild == null || !ignoreChild.Any(ic => ic == child))
            {
                foreach (var c in child.GetComponentsInChildren<UnityEngine.UI.Button>())
                {
                    c.interactable = enabled;
                    SetUguiButtonsEnabledInChildren(c.transform, enabled);
                }
            }
...
//	public static void SetNguiColliderEnabledInChildren(Transform transform, bool enabled)
//	{

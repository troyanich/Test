...
#else
	protected int cursorPosition { get { return (isSelected && mEditor != null) ? mEditor.cursorIndex : value.Length; } }
#endif
		...
		if (label != null && NGUITools.GetActive(this))
		{
			...
			)
			{
				mKeyboard = (inputType == InputType.Password) ?
                    ...
                    TouchScreenKeyboard.Open(mValue, (TouchScreenKeyboardType)((int)keyboardType), false, false, false, true);
			}
			else
#endif
            {
	...
	bool ProcessEvent (Event ev)
	{
		...
		switch (ev.keyCode)
		{
			...
			case KeyCode.KeypadEnter:
			{
				...
				{
					char c = '\n';
					if (onValidate != null) c = onValidate(mEditor.content.text, mEditor.cursorIndex, c);
					else if (validation != Validation.None) c = Validate(mEditor.content.text, mEditor.cursorIndex, c);
